...
class GrnController extends BaseController
{
    ...
    {
        return View::make('grn.index');
    }

    public function anyCreate()
    {
        return View::make('grn.create');
    }

    public function anyGrnlist()
    {

        $grn = WarehouseGrn::lists();
        ...
        return Datatables::of($grn)
                        ...
                        ->add_column('Action', function ($g) {
                            ...
                            return '
                                <a class="btn btn-success" title="" data-toggle="tooltip" href="/grn/download/'.$encrypted.'" target="_blank"><i class="fa fa-download"></i></a>
                                <a class="btn btn-primary" title="" data-toggle="tooltip" href="/grn/edit/'.$g->id.'"><i class="fa fa-pencil"></i></a>
                                <a id="deleteGRN" class="btn btn-danger" title="" data-toggle="tooltip" data-value="'.$g->id.'" href="/grn/delete/'.$g->id.'"><i class="fa fa-remove"></i></a>
                            ';
                        })
                        ->make();
    }
    ...
    {
        $validator = Validator::make(Input::all(), WarehouseGrn::$rules);
        ...
        if ($validator->passes()) {
            ...
            try {
                ...
                if($grn->save())
                {
                    ...
                    $orginal_quantity = Input::get('qty');
                    $quantitys = Input::get('stockin_qty');
                    ...
                    for ($i = 0; $i < count($skus); $i++) { 
                        if($orginal_quantity[$i]==$quantitys[$i]){
                              $scheckin_status=1;
                            }else{
                               $scheckin_status=$cus_status[$i];
                            }
                            $calctotal=$quantitys[$i]* $prices[$i];
                        ...
                        $product = array(
                            ...
                            'price' => $prices[$i],
                            'quantity' => $orginal_quantity[$i],
                            'stockin_qty' => $quantitys[$i],
                            'total' => $calctotal,
                            'foc_qty' => $focQtys[$i],
                            'foc_uom' => $focUoms[$i],
                            'remarks' => $remark[$i],
                            'status' => $scheckin_status,
                            'sst' => $ssts[$i]
                        );
                        $auto_complete[]=$scheckin_status;
                        ...
                        if($cus_status[$i]!='2' || $quantitys[$i]>0){
                        ...
                        $stock_in_history=DB::table('jocom_warehouse_grn_stock')->insert(['grn_id'=>$grn->id,'sku'=>$skus[$i],'stock_in_qty'=>$quantitys[$i],'product_name'=>$productNames[$i],'product_label'=>$priceLabels[$i],'po_id'=>Input::get('po_id'),'grn_no'=>$grn->grn_no,'remarks'=>$remark[$i],'updated_user'=>Session::get('username'),'updated_date'=>date('Y-m-d H:i:s')]);
                        WarehouseController::savestockingrn($productidgrn->id,$unitgrn,$expirydategrn,$unitpricegrn,$totalamountgrn,$remarksgrn);
                    }
                    if($quantitys[$i]>0 && $cus_status[$i]=='2'){
                       $grn->status=3;
                       $grn->save();
                       }
                        
                    }
                    DB::table('jocom_warehouse_grn_details')->insert($products);
                    $po = PurchaseOrder::find(Input::get('po_id'));
                    $po->grn_id = $grn->id;
                    $po->save();
                }
                 if(count(array_unique($auto_complete))==1){
                    $grn->status=1;
                    $grn->save();
                }
                ...
                if ($isError) {
                    ...
                    DB::commit();
                    return Redirect::to('/grn')->with('success', 'Warehouse GRN(GRN Number: '. $grn->grn_no .') added successfully.');
                }
            }
        } else {
    ...
    public function anyEdit($id) {
        ...
        return View::make('grn.edit')->with(array(
            'grn' => $grn,
    ...
    {
        $validator = Validator::make(Input::all(), WarehouseGrn::$updateRules);
            ...
            if($grn->save()) {
                 ...
                 $ids = Input::get('sin_id');
                    ...
                    for ($i = 0; $i < count($ids); $i++) {
                        ...
                        if($cus_status[$i]!='2' || $quantitys[$i]>0){
                        $check_quantity=DB::table('jocom_warehouse_grn_details')
                               ->select('quantity','stockin_qty') 
                               ->where('id','=',$ids[$i])
                               ->first();
                        $total_delivery_quantity=$check_quantity->stockin_qty+$quantitys[$i];
                        if($orginal_quantity[$i]==$total_delivery_quantity){
                              $scheckin_status=1;
                            }else{
                               $scheckin_status=$cus_status[$i];
                            }

                        $detail=DB::table('jocom_warehouse_grn_details')
                               ->select('*') 
                               ->where('id','=',$ids[$i])
                               ->first();
                            
                        $grn_no_detail=DB::table('jocom_warehouse_grn')
                               ->select('grn_no') 
                               ->where('id','=',Input::get('grn_id'))
                               ->first();
                        $totals=$quantitys[$i]*$detail->price;
                        $final_total=$detail->total+$totals;
                DB::table('jocom_warehouse_grn_details')
                ->where('id','=',$ids[$i])
                ->update(['quantity' => $orginal_quantity[$i],
                           'stockin_qty' =>$total_delivery_quantity,
                            'total' => $final_total,
                            'foc_qty' => $focQtys[$i],
                            'foc_uom' => $focUoms[$i],
                            'remarks' => $remark[$i],
                      ...
                      $auto_complete[]=$scheckin_status;
                       ...
                       $stock_in_history=DB::table('jocom_warehouse_grn_stock')->insert(['grn_id'=>$detail->grn_id,'sku'=>$detail->sku,'stock_in_qty'=>$quantitys[$i],'product_name'=>$detail->product_name,'product_label'=>$detail->price_label,'po_id'=>$grn->po_id,'grn_no'=>$grn->grn_no,'remarks'=>$remark[$i],'updated_user'=>Session::get('username'),'updated_date'=>date('Y-m-d H:i:s')]);
...
if(count(array_unique($auto_complete))==1){
                ...
                General::audit_trail('GrnController.php', 'update()', 'Update GRN', Session::get('username'), 'CMS');
                return Redirect::to('/grn')->with('success', 'GRN(ID: '.$grn->id.') updated successfully.');
            }
    ...
    public function anyDelete($id) 
    {
        ...
        $insert_audit = General::audit_trail('GrnController.php', 'delete()', 'Delete GRN', Session::get('username'), 'CMS');

        return Redirect::to('/grn')->with('success', 'GRN(ID: '.$po->id.') deleted successfully.');
    }

    public function anyGrnproducts()
    {
        return View::make('grn.grnproducts');
    }
    ...
    public function anyDownload($loc){
        ...
        $response =  View::make('grn.grn_view')
                        ->with('issuer', $GRNView['issuer'])
                        ->with('grn', $GRNView['grn'])
                        ->with('seller', $GRNView['seller'])
                        ->with('warehouse', $GRNView['warehouse'])
    ...
    public static function createGRNView($grn) {
        ...
        $product_details = DB::table('jocom_warehouse_grn_details')
                            ->where('grn_id', '=', $grn->id)
                            ->get();
    ...
    public function anyProducts() {
        return View::make('grn.ajaxproduct');
    }
    ...
    public function anyProductajax() {
        ...
        // ->edit_column('price_promo', '{{number_format($price_promo, 2)}}')
            ...
            ->add_column('Action', '<a id="selectItem" class="btn btn-primary" title="" href="/grn/prices/{{$id}}">Select</a>')
            ->make();
    }
    ...
    public function anyPrices($id) {
        ...
        return View::make('grn.ajaxprice')->with([
            'id'     => $id,
    ...
    public function anyPbx() {
        $today = date('Y-m-d');
        return View::make('grn.pbx')->with('today', $today);;
    }
    ...
    public function anyPbxlist() {
        ...
        $actionBar = '@if($status==1)<a class="btn btn-primary" title="" data-toggle="tooltip" href="/grn/pbxdownload/{{$file_name}}" target="_blank"><i class="fa fa-download"></i></a> <a class="btn btn-success" title="" data-toggle="tooltip" href="#" onclick="complete_account({{$id;}});"><i class="fa fa-check"></i></a>@endif @if($status==0)Contact IT Dept @endif';
    ...
    public function anyPbxgeneratezip() {
        ...
        $grnsDB = DB::table('jocom_warehouse_grn as grn')
                ->join('jocom_warehouse_grn_details as details', 'grn.id', '=', 'details.grn_id')
    ...
    {
        // $grn = WarehouseGrn::lists()->get();

        // dd($grn);
        return View::make('grn.dashboard');
        // return View::make('grn.dashboard')->with('grns',$grn);
    }
    ...
    public function anyGrnReport()
    {
        ...
        switch ($time_period) {
          case 'Daily':
              $query = WarehouseGrn::lists()
                    ->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id')
        ...
        $grn = $query->select('grn.grn_no',DB::raw('DATE(grn.grn_date)'),'grn_details.sku','grn_details.product_name','grn_details.price_label','grn_details.uom','grn_details.price','grn_details.quantity','grn_details.stockin_qty', 'seller.company_name as company_name','grn_details.status');
// dd($time_period,$start_date,$end_date,$grn->first());
    ...
    public function anyGrnData(){
      ...
      switch (Input::get('time_period')) {
              case 'Daily':
                  $query = WarehouseGrn::lists()->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id')->where(DB::raw('DATE(grn.grn_date)'),$start_date);
                  $query_completed = WarehouseGrn::lists()->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id')->where(DB::raw('DATE(grn.grn_date)'),$start_date)->where('grn.status','=',1);
                  $query_partial = WarehouseGrn::lists()->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id')->where(DB::raw('DATE(grn.grn_date)'),$start_date)->where('grn.status','=',3);
                  $query_percent = WarehouseGrn::lists()->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id');

                  $grn_total_old = $query_percent
                              ->where(DB::raw('DATE(grn.grn_date)'),'<',$start_date)
                              ->distinct('grn.grn_no')
                              ->count('grn.grn_no');
                  $grn_total_new = WarehouseGrn::lists()->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id')
                              ->where(DB::raw('DATE(grn.grn_date)'),'<=',$start_date)
                              ->distinct('grn.grn_no')
                              ->count('grn.grn_no');

                  $grn_old = WarehouseGrn::lists()->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id')
                                  ->selectRaw('ifnull(sum(grn_details.stockin_qty),0) as grn_quantity, ifnull(sum(grn_details.total),0) as grn_amount')
                                  ->where(DB::raw('DATE(grn.grn_date)'),'<',$start_date)
                                  ->first();
                  $grn_new = WarehouseGrn::lists()->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id')
                                  ->selectRaw('ifnull(sum(grn_details.stockin_qty),0) as grn_quantity, ifnull(sum(grn_details.total),0) as grn_amount')
                                  ->where(DB::raw('DATE(grn.grn_date)'),'<=',$start_date)
                                  ->first();
              ...
              case 'Weekly':
                  ...
                  $prv_date->addDays(-7);
                  $query = WarehouseGrn::lists()->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id')
                  ->where(DB::raw('DATE(grn.grn_date)'),'>=',$start_date)
                  ->where(DB::raw('DATE(grn.grn_date)'),'<=',$end_date);
                  $query_completed = WarehouseGrn::lists()->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id')
                  ->where(DB::raw('DATE(grn.grn_date)'),'>=',$start_date)
                  ->where(DB::raw('DATE(grn.grn_date)'),'<=',$end_date)
                  ->where('grn.status','=','1');
                  $query_partial = WarehouseGrn::lists()->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id')
                  ->where(DB::raw('DATE(grn.grn_date)'),'>=',$start_date)
                  ->where(DB::raw('DATE(grn.grn_date)'),'<=',$end_date)
                  ->where('grn.status','=','3');
                  $query_percent = WarehouseGrn::lists()->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id');
              ...
              case 'Monthly':
                ...
                break;
              case 'Custom':
                  $query = WarehouseGrn::lists()->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id')->where(DB::raw('DATE(grn.grn_date)'),'>=',$start_date)->where(DB::raw('DATE(grn.grn_date)'),'<=',$end_date);
                  $query_percent = WarehouseGrn::lists()->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id');
                  $query_completed = WarehouseGrn::lists()->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id')->where(DB::raw('DATE(grn.grn_date)'),'>=',$start_date)->where(DB::raw('DATE(grn.grn_date)'),'<=',$end_date)->where('grn.status','=','1');
                  $query_partial = WarehouseGrn::lists()->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id')->where(DB::raw('DATE(grn.grn_date)'),'>=',$start_date)->where(DB::raw('DATE(grn.grn_date)'),'<=',$end_date)->where('grn.status','=','3');
              ...
              default:
                $query = WarehouseGrn::lists()->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id')->where(DB::raw('DATE(grn.grn_date)'),\Carbon\Carbon::today());
                $query_percent = WarehouseGrn::lists()->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id');
                $query_completed = WarehouseGrn::lists()->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id')->where(DB::raw('DATE(grn.grn_date)'),\Carbon\Carbon::today())->where('grn.status','=','1');
                $query_partial = WarehouseGrn::lists()->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id')->where(DB::raw('DATE(grn.grn_date)'),\Carbon\Carbon::today())->where('grn.status','=','3');
      ...
      $grn_quantity_amount = $query
                    ->selectRaw('ifnull(sum(grn_details.stockin_qty),0) as grn_quantity, ifnull(sum(grn_details.total),0) as grn_amount')->first();
    ...
    public function anyDownloadexcel($type)
    {
      ...
      switch ($time_period) {
        ...
        $data_array[] = array('GRN NO','SKU','PRODUCT NAME','LABEL','UOM','BASE UNIT','PACKING FACTOR','PRICE','TOTAL QUANTITY','TOTAL STOCK IN QTY','TOTAL AMOUNT','FOC','FOC QUANTITY','FOC UOM','COMPANY NAME','STATUS');
        ...
        foreach ($data as $key => $value) {
          ...
          $data_array[] = array(
            ...
            'TOTAL STOCK IN QTY' => $value->stockin_qty,
        ...
        return Excel::create('Product Details', function($excel) use ($data, $data_array,$start_date,$end_date,$time_period) {
            ...
            $excel->sheet('Product Details', function($sheet) use ($data,$data_array,$start_date,$end_date,$time_period)
            {
                ...
                $empty_row_no = 7;
                  ...
                  foreach ($data as $key => $value) {
                    $data_array[] = array('GRN NO','SKU','PRODUCT NAME','LABEL','UOM','BASE UNIT','PACKING FACTOR','PRICE','TOTAL QUANTITY','TOTAL STOCK IN QTY','TOTAL AMOUNT','FOC','FOC QUANTITY','FOC UOM','STATUS');
                ...
                $Status="Completed";
                      ...
                      $data_array[] = array(
                        ...
                        'TOTAL STOCK IN QTY' => $grn_detail->stockin_qty,
            ...
            if($value->status=="3"){
             ...
             $status="Pending";
                    ...
                    $sheet->cell('A'.$heading_row_no.':O'.$heading_row_no,function($cell){
                      $cell->setValignment('center');
                      $cell->setAlignment('center');
                      $cell->setFontWeight('bold');
                    });

                    $sheet->cell('A'.$heading_row_no.':O'.$heading_row_no, function($cell) {$cell->setFont(array(
                      'bold' => true
                    ));   });

                    $record_count = $heading_row_no + count($data_array) - 1;
                    $range_heading = "A".$heading_row_no.":O".$heading_row_no;
                    $data_row_no = $heading_row_no + 1;
                    $range = "A".$data_row_no.":O".$record_count;
                    // $range = 'A7:N10';
    ...
    {
        // return View::make('grn.pdf_view_grn_dash');
        $start_date = Carbon\Carbon::parse(Input::get('start_date'));
        $end_date = Carbon\Carbon::parse(Input::get('end_date'));
        $time_period = Input::get('time_period');
        switch ($time_period) {
        ...
        $grns = $query->selectRaw('grn.id, grn.grn_no, sum(grn_details.quantity) as grn_quantity,sum(grn_details.stockin_qty) as stockin_qty, sum(grn_details.total) as grn_amount, seller.company_name,grn.status')
        ->groupBy(DB::raw('grn.grn_no'))
        ->get();

        // $grns = WarehouseGrn::lists()
        //             ->join('jocom_warehouse_grn_details as grn_details', 'grn.id', '=', 'grn_details.grn_id')
    ...
    public function anyGrnDashboardChildData()
    {
        ...
        $grn_details = WarehouseGrn::lists()
                        ...
                        ->select('grn.id','grn.grn_no','grn_details.sku','grn_details.product_name','grn_details.price_label','grn_details.uom','grn_details.price','grn_details.quantity','grn_details.stockin_qty', 'grn_details.total', 'seller.company_name as company_name','grn_details.status')->get();
        return $grn_details;
    ...
    public function anyViewrecord(){
    
       $id=Input::get('id');
       $sku=Input::get('sku');
       
       $record=DB::table('jocom_warehouse_grn_stock')
            ->select('*')
            ->where('grn_id','=',$id)
            ->where('sku','=',$sku)
            ->get();
        $list= json_decode(json_encode($record), true);
