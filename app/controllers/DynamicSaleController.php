...
class DynamicSaleController extends BaseController
{
    ...
    public function anyIndex(){
        ...
        $products = DB::table('jocom_products')
            ...
            $list = DB::table('jocom_dynamic_sale AS JF')
        ...
        return View::make('jcmdynamicsale.dynamicsale')->with('products',$products)->with('list',$list);
    }
    ...
    public function anyProductsajax()
    {
        ...
        // ->edit_column('price_promo', '{{number_format($price_promo, 2)}}')
            ...
            ->add_column('Action', '<a id="selectItem" class="btn btn-primary" title="" href="/jcmdynamicsale/ajaxprice/{{$id}}">Select</a>')
            ->make();
    }

    public function anyAjaxproduct()
    {
        return View::make('jcmdynamicsale.ajaxproduct');
    }
    ...
    public function anyAjaxprice($id)
    {
        ...
        return View::make('jcmdynamicsale.ajaxprice')->with([
            'id'        => $id,
    ...
    {
        $flash         = new DynamicSale;
        $validator    = Validator::make(Input::all(), DynamicSale::$rules);

        if ($validator->passes()) {
            
            // $title = Input::file('title_filename');

            if (Input::hasFile('title_filename')) {
                

                $title = Input::file('title_filename');
                $destination_path = 'dynamic/images/';
                $attach =  date("Ymdhis")."-".uniqid();
                $filename = $attach. '_' . Input::file('title_filename')->getClientOriginalName();
                $mim=Input::file('title_filename')->getMimeType();
                Input::file('title_filename')->move($destination_path,$filename);

                // $title->move($destination_path, $filename);
          ...
          $flash->type   = "DynamicSale";
          $flash->valid_from  = Input::get('valid_from');
          $flash->valid_to    = Input::get('valid_to');
          $flash->title_filename    = $filename;
          $flash->title_mime        = $mim;
          ...
          if($flash->save()) 
          {
            ...
            if (isset($label_id)) {
                foreach($label_id as $indx => $value) {
                    ...
                    DynamicSaleProducts::scopeStoreProducts($lid,$lbl,$actualprice,$promoprice,$productqty,$limitqty,$productid,$productseq,$id);     
                } 
            }
            
            return Redirect::to('/jcmdynamicsale')->with('message', 'Added successfully.');
          }

        }else{    
            return Redirect::to('/jcmdynamicsale')->withErrors($validator)->withInput();
        }
    }
    ...
    public function anyEdit($id)
    {
        ...
        $flash_products = DB::table('jocom_dynamicsale_products AS JFP')
        ...
        return View::make('jcmdynamicsale.dynamicsale_edit')->with(array(
            'flash'          => $flash,
    ...
    public function anyUpdate($id){
        ...
        $flash = DynamicSale::find($id);
    
        $validator    = Validator::make(Input::all(), DynamicSale::$rulesedit);
        ...
        if ($validator->passes()) {
            ...
            if($flash->save()) 
            {
                ...
                $ProductSeller = DynamicSaleProducts::where("fid",$id)
                ...
                foreach ($ProductSeller as $key => $value) {
                    ...
                    if(!in_array($value->label_id, $label_id)){
                        ...
                        $ProductSellerInfo = DynamicSaleProducts::find($value->id);
                    ...
                    $productqty="0";
                       ...
                       $limits=$limitsqty[$arrayKey]; //added by boobalan
                        DB::table('jocom_dynamicsale_products')
                            ->where('id','=',$value->id)
                            ->update(array(
                                "promo_price"=> $promoprice,
                                "seq"=> $productseq,
                                "qty"=> $productqty,
                                "limit_quantity"=>$limits,
                                "activation"=> 1,
                        ...
                        DB::table('jocom_dynamicsale_stock')
                        ->insert(array(
                            'stock'=>$productqty, 
                            'fpid'=>$value->id,
                ...
                if(count($label_id)> 0){
                    ...
                    $productqty="0";
                        ...
                        $ProductSeller = new DynamicSaleProducts;
                ...
                return Redirect::to('/jcmdynamicsale')->with('message', '(ID: '.$id.') updated successfully.');
            }
        }else{    
            return Redirect::to('/jcmdynamicsale/edit/'.$id)->withErrors($validator)->withInput();
        }
    ...
    public function anyDelete($id)
    {
        ...
        return Redirect::to('/jcmdynamicsale')->with('message','(ID: '.$id.') deleted successfully.');
    }
    
    
    public function check_flashsales_stock($flash_sale_id,$option_id,$qty){
        $checkout_datetime = date("Y-m-d H:i:s");
        $flash_sale_record = DB::table('jocom_dynamic_sale AS JF')
    ...
    public function deduct_flashsales_stock($transaction_id){
        ...
        $flash_sale_record = DB::table('jocom_dynamicsale_transaction_product AS JF')->where("transaction_id",$transaction_id);
        ...
        if($flash_sale_record->count() > 0){
            ...
            foreach($flash_sale_record as $key => $value){
                ...
                $SaleProducts = DynamicSaleProducts::find($id);
