...
// YH: testing controller that use on live server to identify which part code cause code break to system
class YHTestController extends BaseController {
	...
	private static $flash_table = ['FS' => 'jocom_flashsale_products', 'EC' => 'jocom_jocomexcorner_products', 'CD' => 'jocom_combodeals', 'DY' => 'jocom_dynamic_sale'];
	...
	private static $c_src = [
		...
		"webasean" => 6,    // Web Checkout Boost Payment
				...
				if(count($CustomerInfo) > 0){ // Account is exist
					...
					if(Input::get('devicetype') != 'web_mycash'){
						Mail::send('emails.welcome', $data, function($message) use ($user) {
							...
							$message->to($user['email'], $user['name'])->subject("[JOCOM]: Welcome new member!");
						});
					}
				}
			}
	...
	private function CREATE_transaction($post = []){
		...
		$freeshipping = (in_array(date('Y-m-d'), ['2023-01-02']) && in_array($post['delivery_state'], ['458004', '458013', '458015']) ? 1 : 0);
		...
		if ($error === false) { // No Error on location proceed to generate the transaction data
			...
			$qtycheck = DB::table('jocom_productrestrict AS JE')->whereIn('JE.product_id', $post['qrcode'])->lists('qty', 'product_id');
			if (count($qtycheck) > 0) {
		...
		if ($error === false) { // no error on product, proceed to checkout
			...
			if(count($z_nozerofee) == 0){ // is freeshipping item only in checkout
				$freeshipping == 1;
				$d_cy = 0; $d_c0 = 1;
			}else{
				...
				$d_c0 = ($firsttimer == 1 || $durian == 1 || $i_eleven == 1 || $freeshipping == 1 || self::$flash_product_only || ($is_klang && (float)$transac_data['total_amount'] >= 120) || ($is_west && (float)$transac_data['total_amount'] >= 200) ? 1 : 0); // is Freeshipping condition
			}

			if($d_cy || $d_c0){
				$delivery_charges = ($d_c0 ? 0 : ($is_klang ? 10 : ($is_west ? 18 : 8.5)));
				$process_fees = 0;
			...
			if(count($transac_data_detail) > 0){
				...
				array_walk($db_transac_detail, function(&$arg) use ($trans_id){ 
					...
					if(!$arg['seller_id']) $arg['seller_id'] = 0; // Hardcode empty value if not found
					if(!$arg['seller_username']) $arg['seller_username'] = ' '; // Hardcode empty value if not found
					unset($arg['qrcode']);
					unset($arg['is_popbox_available']);
					unset($arg['is_taxable']);
				});
				DB::table('jocom_transaction_details')->insert($db_transac_detail);
			}
	...
	private function ADD_trans_detail($prow, $platform_price, $platform_original_price, $price_option, $qty, &$returnData = [], &$transac_data = [], &$transac_data_detail = [], &$error = false, $type = "", $buyer_username = "") {
		...
		if ($dl_row != null && $price_row != null && !$error) { // Product Price and Delivery fees is avaliable proceed to create the Transaction Details Data
			...
			// Disable Testing Print Code
			// $returnData['error_product'] = [
			// 	'p_id' => $prow['id'],
			// 	'po' => $price_option,
			// 	'po_row' => $price_row,
			// 	'dl_row' => $dl_row,
