...
class JocomExcCornerController extends BaseController
{
    ...
    public function anyIndex(){
        ...
        $products = DB::table('jocom_products')
            ...
            $list = DB::table('jocom_jocomexcorner AS JF')
        ...
        return View::make('jocomexccorner.jcmexcorner_sales')->with('products',$products)->with('list',$list);
    }
    ...
    public function anyProductsajax()
    {
        ...
        // ->edit_column('price_promo', '{{number_format($price_promo, 2)}}')
            ...
            ->add_column('Action', '<a id="selectItem" class="btn btn-primary" title="" href="/jocomexccorner/ajaxprice/{{$id}}">Select</a>')
            ->make();
    }

    public function anyAjaxproduct()
    {
        return View::make('jocomexccorner.ajaxproduct');
    }
    ...
    public function anyAjaxprice($id)
    {
        ...
        return View::make('jocomexccorner.ajaxprice')->with([
            'id'        => $id,
    ...
    {
        $flash         = new JocomExclusiveCorner;
        $validator    = Validator::make(Input::all(), JocomExclusiveCorner::$rules);

        if ($validator->passes()) {
          $flash->main_title   = Input::get('main_title');
          $flash->rule_name   = Input::get('rule_name');
          $flash->type   = "JocomExclusiveCorner";
          ...
          if (Input::hasFile('banner_filename')) {
                ...
                $destination_path = 'exclusivecorners/banner/';
          ...
          if($flash->save()) 
          {
            ...
            if (isset($label_id)) {
              ...
              $productqty="0";  
                    ...
                    JocomExclusiveCornerProducts::scopeStoreProducts($lid,$lbl,$actualprice,$promoprice,$productqty,$limitqty,$productid,$productseq,$id);     
                } 
            }
            
            return Redirect::to('/jocomexccorner')->with('message', 'Added successfully.');
          }

        }else{    
            return Redirect::to('/jocomexccorner')->withErrors($validator)->withInput();
        }
    }
    ...
    public function anyEdit($id)
    {
        ...
        $flash_products = DB::table('jocom_jocomexcorner_products AS JFP')
        ...
        return View::make('jocomexccorner.jcmexcorner_edit')->with(array(
            'flash'          => $flash,
    ...
    public function anyUpdate($id){
        ...
        $flash = JocomExclusiveCorner::find($id);
        $validator    = Validator::make(Input::all(), JocomExclusiveCorner::$rules);
        ...
        if ($validator->passes()) {
            ...
            if($flash->save()) 
            {
                ...
                $seq = Input::get('seq');
                $limitsqty=Input::get('limit');

                $ProductSeller = JocomExclusiveCornerProducts::where("fid",$id)
                ...
                foreach ($ProductSeller as $key => $value) {
                    ...
                    if(!in_array($value->label_id, $label_id)){
                        ...
                        $ProductSellerInfo = JocomExclusiveCornerProducts::find($value->id);
              ...
              $productqty ="0"; 
                        ...
                        $limitqty = $limitsqty[$arrayKey];
                        DB::table('jocom_jocomexcorner_products')
                            ->where('id','=',$value->id)
                            ->update(array(
                        ...
                        DB::table('jocom_jocomexcorner_stock')
                        ->insert(array(
                            'stock'=>$productqty, 
                            'fpid'=>$value->id,
              ...
              $productqty ="0"; 
                        ...
                        $limitqty=$limitsqty[$indx];
                        $productseq = $seq[$indx];
                        $ProductSeller = new JocomExclusiveCornerProducts;
                        ...
                        $ProductSeller->qty = $productqty;
                        $ProductSeller->limit_quantity = $limitqty;
                        $ProductSeller->seq = $productseq;
                ...
                return Redirect::to('/jocomexccorner')->with('message', '(ID: '.$id.') updated successfully.');
            }
        }else{    
            return Redirect::to('/jocomexccorner/edit/'.$id)->withErrors($validator)->withInput();
        }
    ...
    public function anyDelete($id)
    {
        ...
        $flash->updated_at = date('Y-m-d h:i:s');
        $flash->save();
        return Redirect::to('/jocomexccorner')->with('message','(ID: '.$id.') deleted successfully.');
    }
    ...
    public function check_flashsales_stock($flash_sale_id,$option_id,$qty){
        ...
        $flash_sale_record = DB::table('jocom_jocomexcorner AS JF')
    ...
    public function deduct_flashsales_stock($transaction_id){
        ...
        $flash_sale_record = DB::table('jocom_jocomexcorner_transaction_product AS JF')->where("transaction_id",$transaction_id);
        ...
        if($flash_sale_record->count() > 0){
            ...
            foreach($flash_sale_record as $key => $value){
                ...
                $SaleProducts = JocomExclusiveCornerProducts::find($id);
