...
class CheckoutV2Controller extends BaseController
{
    /**
...
//                      }
   //till here by sairam  
        ...
        if(date("Y-m-d H:i:s") >= Config::get('constants.NEW_INVOICE_SST_START_DATE')){
            /* SST TEMPLATE */
            $checkout_view    = (Input::get('devicetype') == "manual") ? '.manual_checkout_v2' : '.checkout_view_v3';
        }  else{
            /* GST TEMPLATE */
            $checkout_view    = (Input::get('devicetype') == "manual") ? '.manual_checkout_v2' : '.checkout_view_v3';
        } 
            ...
            */
            ...
            if (isset($data['status']) && $data['status'] == 'success') {
                ...
                if (isset($data['trans_query'])) {
                    ...
                    $data['points'] = $points;
                    // start section for delivery address change and checkout coupon
                    $data['static_coupon']=self::getCheckoutcoupon($data["transaction_id"]);
                    $addresss = FavouriteAddress::where('username', '=', $_POST["user"])->get();
                    
                    if (count($addresss) > 0){
                        $data['record'] = count($addresss);
                    ...
                    //end section for delivery address and checkout coupon
...
try{
        ...
        if (isset($_POST["transaction_id"])) {
            ...
            /* INCLUSIVE TEMPLATE */		
                $checkout_view    = ($_POST['devicetype'] == "manual") ? '.manual_checkout_v2' : '.checkout_view_v3';		
            }  else{		
                /* EXCLUSIVE TEMPLATE */		
            ...
            if(!empty($alert)){
               ...
               Session::put('coupon_msg', "{$hint}");
                ...
                $data['points'] = $points;
                $data['static_coupon']=self::getCheckoutcoupon($data["transaction_id"]);
                    $addresss = FavouriteAddress::where('username', '=',$Transaction->buyer_username)->get();
            ...
            //check coupon code
                ...
                Session::put('coupon_msg', "Oops! You will not be able to use this voucher with tmGrocer preferred member package.");
                $buyerId        = $data['trans_query']->buyer_id;
                $points         = PointUser::getPoints($buyerId, PointUser::ACTIVE_ONLY);
                $data['points'] = $points;
                $data['static_coupon']=self::getCheckoutcoupon($data["transaction_id"]);
            ...
            if($lscheck == 0){
                ...
                if(strtoupper($_POST["coupon"]) == 'NEW15') {
            ...
            if($member_coupon=='PMPFD10' ||$member_coupon=='PMP18'||$member_coupon=='PMP25'){
             ...
             {
                   Session::put('coupon_msg','We are regret to inform you that you are not eligible to use it! Please Purchase tmGrocer preferred membership');
                $buyerId        = $data['trans_query']->buyer_id;
                $points         = PointUser::getPoints($buyerId, PointUser::ACTIVE_ONLY);
                $data['points'] = $points;
                $data['static_coupon']=self::getCheckoutcoupon($data["transaction_id"]);
                    ...
                    if (count($addresss) > 0){
                        ...
                        if($_POST['devicetype'] == "web"){
                            $data['coupon_msg'] = 'We are regret to inform you that you are not eligible to use it! Please Purchase tmGrocer preferred membership';
                             return $data;
                        }else{
                        return Response::view(Config::get('constants.CHECKOUT_FOLDER').$checkout_view, $data);
                        }  
    ...
    try{
        ...
        if (isset($_POST["transaction_id"])) {
            ...
            /* INCLUSIVE TEMPLATE */        
             ...
             //Temp.    
                $checkout_view    = ($_POST['devicetype'] == "manual") ? '.checkout_view_v3' : '.checkout_view_v3';        
...
//                die();
    ...
    public function postPointredemp(){
        ...
        $checkout_view    = ($transactionDate >= Config::get('constants.NEW_INVOICE_V2_START_DATE') ? (Input::get('devicetype') == "manual" ? '.manual_checkout_v2' : '.checkout_view_v3' ) : (Input::get('devicetype') == "manual" ? '.manual_checkout' : '.checkout_view'));
    ...
    public function anyMolxml()
    {
        ...
        if(isset($_POST['devicetype']) == 'android' ){ // ANDROID
            ...
            $tran_data =  array(
                ...
                "bill_desc"=>"Payment for tmGrocer",
            );
        ...
        public function anyUpdatfaveaddress()
    {
      ...
      public function anyFavaddressupdate()
    {
    try{
        ...
        $transaction_id=$_POST['transaction_id'];
        $address_id=$_POST['id'];
        $finduser= Transaction::find($transaction_id);
        $addr = FavouriteAddress::where('id', '=',$address_id)->where('username','=',$finduser->buyer_username)->first();
        $returnData=array();
        ...
        if (count($addr) > 0 && $error===false)
        {
            ...
            $data['points'] = $points;
            $data['transaction_id'] = $_POST["transaction_id"];
            $data['addressmsg'] = Session::get('addressmsg');
            Session::put('addressmsg',"Delivery Address Changed Successfully!");
            $today=date("Y-m-d");
            $coupon_static=DB::table('jocom_static_coupon as sc')
                           ...
                           ->orderBy('sc.id','DESC')
                           ->get();
            $data['static_coupon']=$coupon_static;
            $addresss = FavouriteAddress::where('username', '=',$Transaction->buyer_username)->get();
            ...
            $data['transaction_id'] = $_POST["transaction_id"];
            $data['addressmsg'] = Session::get('addressmsg');
              Session::put('addressmsg', "Something went wrong!Please try again");
            $today=date("Y-m-d");
            $coupon_static=DB::table('jocom_static_coupon as sc')
                           ->select('sc.id','sc.coupon_id','sc.coupon_code', 'sc.coupon_amount','sc.coupon_amount_type','sc.description','sc.status')
                           ->join('jocom_coupon as jc ','sc.coupon_id','=','jc.id')
            ...
            $data['transaction_id'] = $_POST["transaction_id"];
            $data['addressmsg'] = Session::get('addressmsg');
            Session::put('addressmsg',$returnData['message']);
            $today=date("Y-m-d");
            $coupon_static=DB::table('jocom_static_coupon as sc')
                           ->select('sc.id','sc.coupon_id','sc.coupon_code', 'sc.coupon_amount','sc.coupon_amount_type','sc.description','sc.status')
                           ->join('jocom_coupon as jc ','sc.coupon_id','=','jc.id')
                   ...
                   if($CustomerInfo->preferred_member==1){
                     $coupon=DB::table('jocom_coupon as sc')
                                   ...
                                   if($usercoupons){
                                     ...
                                     $users_totalcoupon=$coupon;  
                                   }
                                  
                                   if(!empty($users_totalcoupon)){
                                       foreach($users_totalcoupon as $value){
                                           if($value->coupon_code=='PMPFD10'){
                                            $CustomerInfo = Customer::where('username',$Transaction->buyer_username)->first();
                                            $available_count='.<br/>Coupon Claim Remaining: '.$CustomerInfo->membership_delivery;
                                           }else if($value->coupon_code=='PMP18'){
                                              $CustomerInfo = Customer::where('username',$Transaction->buyer_username)->first();
                                            $available_count='.<br/> Coupon Claim Remaining: '.$CustomerInfo->member_disc_1; 
                                           }else if($value->coupon_code=='PMP25'){
                                             $CustomerInfo = Customer::where('username',$Transaction->buyer_username)->first();
                                            $available_count='.<br/> Coupon Claim Remaining: '.$CustomerInfo->member_disc_2;   
                                           }else{
                                             $available_count='';  
                                           }
                                           $combained[]=array(
