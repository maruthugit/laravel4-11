...
class ApiLogisticController extends BaseController {
    ...
    */
    ...
    public function anyOverallstatics(){
        
            $overallstatistics=array();    
            ...
            $overallstatistics=array(
                'overall'=>array(array('total'=>count($TotalPendingAllplace),'id'=>'801','title'=>'Pending Place'),array('total'=>count($TotalSendingAllplace),'id'=>'802','title'=>'Sending Place'),
                array('total'=>$TotalPendingAll,'id'=>'803','title'=>'Pending'),
                array('total'=>$TotalSendingAll,'id'=>'804','title'=>'Sending'),
                array('total'=>$TotalUndeliveredAll,'id'=>'805','title'=>'Undelivered'),
                array('total'=>$TotalPartialAll,'id'=>'806','title'=>'Partial sent'),
                array('total'=>$TotalReturnedAll,'id'=>'807','title'=>'Returned'),
                array('total'=>$TotalSentAll,'title'=>'sent')),
                'weekly_pending'=>array(
                    array('total'=>$totalPendingDelay1Month,'id'=>'809','title'=>'1 week','month'=>1),
                    array('total'=>$totalPendingDelay2Month,'id'=>'809','title'=>'3 weeks','month'=>2),
                    array('total'=>$totalPendingDelay3Month,'id'=>'809','title'=>'Over 1 month','month'=>3)),
                'weekly_sending'=>array(
                    array('total'=>$totalSendingDelay1Month,'id'=>'810','title'=>'1 week','month'=>1),
                    array('total'=>$totalSendingDelay2Month,'id'=>'810','title'=>'3 weeks','month'=>2),
                    array('total'=>$totalSendingDelay3Month,'id'=>'810','title'=>'Over 1 month','month'=>3)),
                'weekly_return'=>array(
                    array('total'=>$totalReturnedDelay1Month,'id'=>'811','title'=>'1 week','month'=>1),
                    array('total'=>$totalReturnedDelay2Month,'id'=>'811','title'=>'3 weeks','month'=>2),
                    array('total'=>$totalReturnedDelay3Month,'id'=>'811','title'=>'Over 1 month','month'=>3)),
                );
        
        return json_encode($overallstatistics);
    }
    
    public function anyLogisticspopupdata(){
        
        $static_id=Input::get('status_id');
        $logistic_data=array();
        if($static_id!=''){
            ...
            switch($static_id){
                ...
                ->groupBy('location')
                ->get();
                foreach($TotalPendingAllplace as $value){
                    
                $logistic_data[]=array('total_order'=>$value->total_order,
                ...
                ->groupBy('location')
                ->get();
                foreach($TotalSendingAllplace as $value){
                ...
                $pendingresult = DB::table('logistic_transaction')
                ->leftJoin("jocom_transaction", 'logistic_transaction.transaction_id', '=', 'jocom_transaction.id')
                ->select("logistic_transaction.*", "jocom_transaction.buyer_username as platform")
                ->where("logistic_transaction.status",0)
                ->get();
                
                  foreach($pendingresult as $value){
                      
                      $platform=array('1Street'=>"11Street",
                      ...
                      if(in_array($value->platform,$platform)){
                          $platform_name=$platform[$value->platform];
                      }else{
                         $platform_name=$platform['jocom']; 
                      }
                    
                      $logistic_data[]=array('transaction_id'=>$value->transaction_id,
                ...
                break;
                case '804':
                $sendingresult = DB::table('logistic_transaction')
                ->leftJoin("jocom_transaction", 'logistic_transaction.transaction_id', '=', 'jocom_transaction.id')
                ->select("logistic_transaction.*")
                ->where("logistic_transaction.status",4)
                ->groupBy('logistic_transaction.id')
                ->get();
                
                 foreach($sendingresult as $value){
                    ...
                    break;
                case '805':
                      $undeliverresult = DB::table('logistic_transaction')
                                ->where("status",1)
                                ->get();
                        foreach($undeliverresult as $value){
                    $logistic_data[]=array('transaction_id'=>$value->transaction_id,
                                       'insert_date'=>$value->insert_date,
                                       'delivery_name'=>$value->delivery_name,
                                       'delivery_city'=>$value->delivery_city,
                  ...
                  break;
                  case '806':
                      $partialsentresult = DB::table('logistic_transaction')
                                ->where("status",2)
                                ->get();
                        foreach($partialsentresult as $value){
                    $logistic_data[]=array('transaction_id'=>$value->transaction_id,
                                       'insert_date'=>$value->insert_date,
                                       'delivery_name'=>$value->delivery_name,
                                       'delivery_city'=>$value->delivery_city,
                      ...
                      break;
                    case '807':
                      $rerurnedresult = DB::table('logistic_transaction')
                                ->where("status",3)
                                ->get();
                        foreach($rerurnedresult as $value){
                    $logistic_data[]=array('transaction_id'=>$value->transaction_id,
                                       'insert_date'=>$value->insert_date,
                                       'delivery_name'=>$value->delivery_name,
                                       'delivery_city'=>$value->delivery_city,
                      ...
                      case '809':
                          ...
                          $status=0;
                          $listresult = LogisticTransaction::getAllListDelay($status,$period);
                          if($listresult!=""){
    ...
    public function anyDriverstatisticlatest(){
        $username = Input::get('username');
        $status = Input::get('status');
        //sent = 4, sending = 1, returned = 2, pending = 0
        $startDate = Input::get("startDate")." 08:00:00";
        ...
        foreach ($LogisticDriver as $keyDriver => $valueDriver) {
          ...
          if($totalAssign!="0" || $totalSending!="0" || $totalReturned!="0" || $totalUndelivered!="0" || $totalSent!="0" || $totalCancelled!="0"){
            array_push($collectionDriver, $driver_temp);
          }
    ...
    public function anyStatisticsbydate(){
            ...
            $toDate = date('Y-m-d', strtotime(date($toDate), time()))." 23:59:59";
            }else{
        ...
        $states = LogisticTransaction::getStates($username);
            ...
            $logistic_data=array(
                'overall'=>array(array('total'=>$TotalPending,'status'=>'0','title'=>'Pending'),
                    array('total'=>$TotalSending,'status'=>'4','title'=>'Sending'),
                    array('total'=>$TotalUndelivered,'status'=>'1','title'=>'Undelivered'),
                    array('total'=>$TotalPartial,'status'=>'2','title'=>'Partial'),
                    array('total'=>$TotalReturned,'status'=>'3','title'=>'Returned'),
                    array('total'=>$TotalSent,'status'=>'5','title'=>'Sent'),
                    array('total'=>$TotalBatchPending,'title'=>'TOTAL BATCH SENDING'),
                    array('total'=>$TotalBatchSending + $TotalBatchReturned + $TotalBatchUndelivered,'title'=>'TOTAL BATCH PROCESSING'),
                    array('total'=>$TotalBatchSent,'title'=>'TOTAL BATCH COMPLETED')));
            
            }else{
            $TotalPending = LogisticTransaction::getTotalRecordByStatusRegions(0,$startDate,$toDate,$username);
            $TotalUndelivered= LogisticTransaction::getTotalRecordByStatusRegions(1,$startDate,$toDate,$username);
            $TotalPartial= LogisticTransaction::getTotalRecordByStatusRegions(2,$startDate,$toDate,$username);
            $TotalReturned = LogisticTransaction::getTotalRecordByStatusRegions(3,$startDate,$toDate,$username);
            $TotalSending= LogisticTransaction::getTotalRecordByStatusRegions(4,$startDate,$toDate,$username);
            $TotalSent= LogisticTransaction::getTotalRecordByStatusRegions(5,$startDate,$toDate,$username);
            
            $TotalBatchPending= LogisticBatch::getTotalRecordByStatusRegions(0,$startDate,$toDate,$username);
            $TotalBatchSending= LogisticBatch::getTotalRecordByStatusRegions(1,$startDate,$toDate,$username);
            $TotalBatchReturned= LogisticBatch::getTotalRecordByStatusRegions(2,$startDate,$toDate,$username);
            $TotalBatchUndelivered= LogisticBatch::getTotalRecordByStatusRegions(3,$startDate,$toDate,$username);
            $TotalBatchSent= LogisticBatch::getTotalRecordByStatusRegions(4,$startDate,$toDate,$username);
    ...
    public function anyLogisticlistbydate(){
        
            $startDate = Input::get("startDate");
            $toDate = Input::get("toDate");
            $logistic_data=array();
            $startDate = date('Y-m-d', strtotime(date($startDate), time()))." 00:00:00";
            $endDate = date('Y-m-d', strtotime(date($toDate), time()))." 23:59:59";
            $status=Input::get('status');
        $username=Input::get('username');
        $user = DB::table('jocom_sys_admin')->where('username','=',$username)->first();
        $region = DB::table('jocom_sys_admin_region AS JSR')
    ...
    public function anyTotalbatchsentcharts(){
            $startDate = Input::get("startDate");
            $toDate = Input::get("toDate");
            $logistic_data=array();
            if($startDate!='' && $toDate!=''){
        ...
        $states = LogisticTransaction::getStates($username);
         if($startDate!='' && $toDate!=''){
            ...
            foreach ($LogisticDriver as $keyDriver => $valueDriver) {
                ...
                if($totalAssign!=0 || $totalSent!=0 || $totalReturn!=0 ){
                array_push($collectionDriver, $driver_temp);
                }
            }
            $logistic_data=$collectionDriver;
             }else{
            ...
            $logistic_data=$collectionDriverRegion;
             }
         }
