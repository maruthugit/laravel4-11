...
<body>
    ...
    <div class="wrapper">
        <div id="page-wrapper" style="margin: 0">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Sellers</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#dataTables-sellers').dataTable({
                "processing" : true,
                "serverSide" : true,
                "ajax" : "{{ URL::to('special_price/sellersajax') }}",
                "order" : [[ 0, 'desc' ]],
                "lengthMenu" : [ 5 ],
                "bLengthChange" : false,
                "columnDefs" : [{
            ...
            $(document).on("click", "#selectSeller", function() {
                ...
                parent.$('#emptyseller').hide();

                var rowTd = $('<tr class="seller">\
                                    <input type="hidden" id="sid[]" name="sid[]" value="'+localStorage.data0+'">\
                                    ...
                                    <td class="text-center col-xs-1">\
                                        <div class="btn-group">\
                                            <a class="btn btn-xs btn-danger" id="deleteSeller" data-toggle="tooltip" href="javascript:void(0)" data-original-title="Delete"><i class="fa fa-times"></i> Remove</a>\
                                        </div>\
                                    </td>\
                                </tr>');

                var rowCount = parent.$('#stb tr').length;
                var exists  = 0;
                    ...
                    for(var k = 0; k < rowCount; k++) {
                        for (var j = 0; j < localStorage.length; j++){
                            ...
                            if (localStorage.key(j) == key) {
                                ...
                                if(item == id){
                                    exists = 1;
                                    bootbox.alert("The seller is already EXISTS! Please select another seller.", function(e){
                ...
                if(exists == 0) {
                    parent.$('#stb').append(rowTd);
                    localStorage.setItem("sid"+ rowCount, id);
                    parent.$.fn.colorbox.close();
                    return false;
                }
            });
