                ...
                <div class="col-lg-12">
                    <div class="form-group">
                        <h3 class="page-header">Product name : {{$name}} (SKU: {{$sku}})
                            <span class="pull-right"><a class="btn btn-default" title="" href="/special_price/ajaxproduct/"><i class="fa fa-reply"></i> Back</a></span>
                        </h3>
                    </div>
                </div>
                <!-- /.col-lg-12 -->
                                ...
                                <table class="table table-striped table-bordered table-hover" id="dataTables-products">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-center col-sm-1">Default</th>                                            
                                            <th class="text-center col-sm-1">Discount Amount</th>                                            
                                            <th class="text-center col-sm-1">Discount Type</th>                                            
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#dataTables-products').dataTable({
                "processing" : true,
                "serverSide" : true,
                "ajax" : "{{ URL::to('special_price/pricesajax/'.$id) }}",
                "order" : [[ 0, 'desc' ]],
                "lengthMenu" : [ 5 ],
                "bLengthChange" : false,
                "columnDefs" : [{
            ...
            $(document).on("click", "#selectItem", function(e) {
                ...
                var rowTd   = '<input type="hidden" name="label[]" value="' + id + '">\
                                <input type="hidden" name="product[]" value="{{$id}}">\
                                <input type="hidden" name="default[]" value="{{$default}}">\
                                <input type="hidden" name="name[]" value="{{$name}}">\
                                <input type="hidden" name="actual_price[]" value="' +  actual_price+ '">\
                                ...
                                <td class="text-center col-xs-1">'+ currencyFormat(parseFloat(actual_price)) +'</td>\
                                <td class="text-center col-xs-1">'+ currencyFormat(parseFloat(promo_price)) +'</td>\
                                <td class="text-center col-xs-1">'+ disc +'</td>\
