...
<body>
    ...
    <div class="wrapper">
        <div id="page-wrapper" style="margin: 0">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Special Price Groups</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                                ...
                                <table class="table table-striped table-bordered table-hover" id="dataTables-group">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="col-sm-3">Seller Company Name</th>
                                            <th class="col-sm-3">Seller Email</th>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#dataTables-group').dataTable({
                "processing" : true,
                "serverSide" : true,
                "ajax" : "{{ URL::to('special_price/groupsajax') }}",
                "order" : [[ 0, 'desc' ]],
                "lengthMenu" : [ 5 ],
                "bLengthChange" : false,
                "columnDefs" : [{
            ...
            $(document).on("click", "#selectGroup", function() {
                ...
                parent.$('#emptygroup').hide();

                var rowTd = $('<tr class="seller">\
                                    <input type="hidden" id="gid[]" name="gid[]" value="'+localStorage.data0+'">\
                                    ...
                                    <td class="text-center col-xs-1">\
                                        <div class="btn-group">\
                                            <a class="btn btn-xs btn-danger" id="deleteGroup" data-toggle="tooltip" href="javascript:void(0)" data-original-title="Delete"><i class="fa fa-times"></i> Remove</a>\
                                        </div>\
                                    </td>\
                                </tr>');

                var rowCount = parent.$('#gtb tr').length;
                var exists  = 0;
                    ...
                    for(var k = 0; k < rowCount; k++) {
                        for (var j = 0; j < localStorage.length; j++){
                            ...
                            if (localStorage.key(j) == key) {
                                ...
                                if(item == id){
                                    exists = 1;
                                    bootbox.alert("The group is already EXISTS! Please select another group.", function(e){
                ...
                if(exists == 0) {
                    parent.$('#gtb').append(rowTd);
                    localStorage.setItem("gid"+ rowCount, id);
                    parent.$.fn.colorbox.close();
                    return false;
                }
            });
