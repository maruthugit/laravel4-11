...
<div id="page-wrapper">
    ...
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header">Brands Management</h1>
        </div>
        <!-- /.col-lg-12 -->
    </div>
    ...
    <div class="row">
        ...
        {{ Form::open(array('url' => array('brands/update', $branditem->id), 'method'=> 'PUT', 'class' => 'form-horizontal', 'files'=> true)) }}
                ...
                <div class="panel-body">
                    <div class="col-lg-12">
                        <div class='form-group'>
                            ...
                            <div class="col-lg-6">
                            {{ Form::text('id', $branditem->id, array('class' => 'form-control', 'disabled') ) }}
                            </div>
                        </div>
                        <!--
                        <div class='form-group'>
                            ...
                            <div class="col-lg-6">
                                <select class="form-control" id="platform" name="platform" style="margin-top: 10px;">
                                    <option value="JOC" <?php if($branditem->is_jocom_app == 1) { echo "selected"; } ?> >JOCOM</option>
                                    <option value="JUE" <?php if($branditem->is_juepin_app == 1) { echo "selected"; } ?> >JUEPIN</option>
                                </select>
                            </div>
                        </div>
                        -->
					...
					{{ Form::label('banner_region', 'Region', ['class' => 'col-lg-2 control-label']) }}
						<div class="col-lg-3">
							<select class="form-control" id="region_country_id" name="region_country_id">
                                <?php foreach ($countries as $key => $value) { ?>
                                    <option value="<?php echo $value->id; ?>" <?php if($branditem->region_country_id == $value->id){ echo "selected";} ?>><?php echo $value->name;?></option>
                                <?php } ?>
                            </select>
                            <select class="form-control" id="region_id" name="region_id" style="margin-top: 10px;">
                                ...
                                <?php foreach ($regions as $key => $value)  { ?>
                                    <option value="<?php echo $value->id; ?>" <?php if($value->id == $branditem->region_id) { echo "selected";} ?>><?php echo $value->region; ?></option>
                                <?php  } ?>
                            </select>
						</div>
                        <div class="col-lg-3">
                        ...
                        <div class='form-group'>
                            ...
                            <div class="col-lg-6">
                            {{ Form::text('insert_date', $branditem->insert_date, array('class' => 'form-control', 'disabled') ) }}
                            </div>
                        </div>
                        <div class="form-group @if ($errors->has('qrcode')) has-error @endif">
                        {{ Form::label('qrcode', 'QR Code', array('class' => 'col-lg-2 control-label')) }}
                            <div class="col-lg-6">
                            {{ Form::text('qrcode', $branditem->qrcode, array('placeholder' => 'e.g. JC2110 or JC1000, JC2000, JC3000', 'class' => 'form-control') ) }}
                         ...
                         <div class='form-group'>
                            ...
                            <div class="col-lg-6">
                            {{ Form::text('pos', $branditem->pos, array('class' => 'form-control') ) }}
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-lg-12 table-responsive">
                        <table class="table table-hover" style="text-align:center;">
                            <thead>
                             ...
                             <tbody>
                        <?php $count = count($branditem_images); ?>
                        @foreach ($languages as $code => $name)
                            <?php $loaded = 0; ?>
                            @if($loaded == 0)
                                @foreach ($branditem_images as $branditem)
                                    @if ($branditem->language == $code && $branditem->device == "phone")
                                        <?php $loaded = 1; ?>
                                        <tr>
                                            <td>{{ Form::label('branditem', $name, array('class' => 'col-lg-2 control-label')) }}</td>
                                            <td>
                                                <div class="fileinput fileinput-new" data-provides="fileinput">
                                                    <div class="fileinput-preview thumbnail" style="max-width: 200px; max-height: 320px; line-height: 20px;">
                                                        {{ $name }} -  {{ $branditem -> file_name }} <br>
                                                        @if(file_exists('./images/brand_items/' . $code .'/' .$branditem->file_name))
                                                            {{ HTML::image('images/brand_items/' . $code. '/' . $branditem->file_name) }}
                                                        @else
                                                            file not found!
                                            ...
                                            <td>
                                                <div class="fileinput fileinput-new" data-provides="fileinput">
                                                    <div class="fileinput-preview thumbnail" style="max-width: 200px; max-height: 150px;">
                                                        {{ $name }} -  {{ $branditem -> thumb_name }} <br>
                                                        @if(isset($branditem->thumb_name))
                                                            @if(file_exists('./images/brand_items/thumbs/'. $code . '/' . $branditem->thumb_name))
                                                                {{ HTML::image('images/brand_items/thumbs/'. $code . '/' . $branditem->thumb_name) }}
                                                            @else
                                                                file not found! 
                                                                {{ HTML::image('media/no_images.jpg') }}
                                                            @endif
                ...
                <div class="panel-body">
                    <div class="col-lg-12 table-responsive">
                        ...
                        @foreach ($languages as $code => $name)
                            ...
                            @if($loaded == 0)
                                @foreach ($branditem_images as $branditem)
                                    @if ($branditem->language == $code && $branditem->device == "tablet")
                                        <?php $loaded = 1; ?>
                                        <tr>
                                            <td>{{ Form::label('branditem', $name, array('class' => 'col-lg-2 control-label')) }}</td>
                                            <td>
                                                <div class="fileinput fileinput-new" data-provides="fileinput">
                                                    <div class="fileinput-preview thumbnail" style="max-width: 200px; max-height: 320px; line-height: 20px;">
                                                        {{ $name }} -  {{ $branditem -> file_name }} <br>
                                                        @if(file_exists('./images/brand_items_tab/' . $code .'/' .$branditem->file_name))
                                                            {{ HTML::image('images/brand_items_tab/' . $code. '/' . $branditem->file_name) }}
                                                        @else
                                                            file not found!
                                                            {{ HTML::image('media/no_images.jpg') }}
                                                        @endif
                                            ...
                                            <td>
                                                <div class="fileinput fileinput-new" data-provides="fileinput">
                                                    <div class="fileinput-preview thumbnail" style="max-width: 200px; max-height: 150px;">
                                                        {{ $name }} -  {{ $branditem -> thumb_name }} <br>
                                                        @if(isset($branditem->thumb_name))
                                                            @if(file_exists('./images/brand_items_tab/thumbs/'. $code . '/' . $branditem->thumb_name))
                                                                {{ HTML::image('images/brand_items_tab/thumbs/'. $code . '/' . $branditem->thumb_name) }}
                                                            @else
                                                                file not found! 
                                                                {{ HTML::image('media/no_images.jpg') }}
                                                            @endif
