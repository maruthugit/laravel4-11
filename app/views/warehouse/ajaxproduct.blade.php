...
<body>
                                ...
                                <table class="table table-striped table-bordered table-hover" id="dataTables-products">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="col-sm-1 text-center">Variant/Related ID</th>
    ...
    <div class="modal fade modal1" id="myModalAll"  tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content"> 
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    <h4 class="modal-title" id="myModalLabel">Select Base Variant/Related Products</h4>
                  </div>
                  
                  <div class="modal-body">

                        <table class="table table-striped table-bordered table-hover" id="dataTables-productsvariation">
                            <thead>
                                <tr>
                                    <th class="col-sm-1 text-center">ID</th>
                                    <th class="col-sm-4">Product Name</th>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(document).on("click", "#selectItem", function() {
                ...
                var currentID = '#variantID'+lid;
                var dropdown = $(currentID).val();
                
                // alert(dropdown);

                var rowTd = $('<tr id="'+lid+'" class="product">\
                                    <input type="hidden" id="parent_productID[]" name="parent_productID[]" value="'+lid+'">\
                                    <input id="node_productName[]" name="node_productName[]" value="'+localStorage.data3+'" type="hidden">\
                                    <input id="node_variantproductID[]" name="node_variantproductID[]" value="'+dropdown+'" type="hidden">\
                                    <input id="node_NodeLevel[]" name="node_NodeLevel[]" value="'+parent.$('#multipleNodeProductsTable tr').length+'" type="hidden">\
                                    <td> '+ lid +'</td>\
                                    <td class="hidden-xs hidden-sm">'+localStorage.data3+'</td>\
                                    <td class="hidden-xs hidden-sm">'+dropdown+'</td>\
                                    <td class="hidden-xs hidden-sm col-xs-1"><input type="text" class="form-control col-xs-2"  id="node_quantity[]"  name="node_quantity[]" value=""></td>\
                                    <td class="text-center col-xs-1">\
                                        <div class="btn-group">\
                                            <a class="btn btn-xs btn-danger" id="deleteItem" data-toggle="tooltip" href="javascript:void(0)" data-original-title="Delete"><i class="fa fa-times"></i> Remove1</a>\
                                        </div>\
                                    </td>\
                                </tr>');
               ...
               // <td class="hidden-xs hidden-sm col-xs-1 text-center r_length">'+parent.$('#multipleNodeProductsTable tr').length+'</td>\   - Removed row Length;
                ...
                var i       = 0;
                var exists  = 0;
                var rowCount = parent.$('#multipleNodeProductsTable tr').length;
                ...
                if (exists == 0) {
                    parent.$('#multipleNodeProductsTable').append(rowTd);
            ...
            $(document).on("change", "#selectItem1", function() {
