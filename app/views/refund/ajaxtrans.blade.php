...
<body>
    ...
    <div class="wrapper">
        <div id="page-wrapper" style="margin: 0">
            <div class="row">
                <div class="col-lg-12">
                    <h3 class="page-header">Refund Management - Add Transaction</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                            ...
                            <div class="table-responsive" style="overflow-x: none">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-trans">
                                    <thead>
                                        <tr>
                                            <th>ID</th>
                                            <th class="col-sm-2">Transaction Date</th>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#dataTables-trans').dataTable({
                "processing" : true,
                "serverSide" : true,
                "ajax" : "{{ URL::to('refund/transactionsajax') }}",
                "order" : [[0,'desc']],
                "lengthMenu" : [ 10 ],
                "bLengthChange" : false,
                "columnsDefs" : [
                ...
                "columns" : [
                    { "data" : "0", "orderable" : false, "className" : "col-sm-1" },
                    { "data" : "1" },
                    { "data" : "2", "className": "buyer_id", "visible" : false },
                    { "data" : "3" },
            ...
            $(document).on("click", "#selectTrans", function() {
                ...
                var c = 0;
                $.each($tds, function() {
                    var data = $(this).attr('data-passdata');

                    if($(this).attr('class') == "buyer_id") {
                        buyer_id = data;
                        localStorage.setItem("buyer_id", buyer_id);
                    }

                    if(c == 3)
                        localStorage.setItem("data" + c, $(this).text().trim());
                    else
                ...
                parent.$('input#trans_id').val(localStorage.data0);
                parent.$('input#trans_date').val(localStorage.data1);
                parent.$('input#buyer').val(localStorage.data2);
                parent.$('input#buyer_id').val(localStorage.buyer_id);
                parent.$('input#trans_amount').val(localStorage.data3);
