...
<div id="page-wrapper">
    <div class="row">
        <div class="col-lg-12" id="headerpanel">
    ...
    <div class="row" style="background-color: #fbfbfb;" >
        <div class="col-lg-3 col-md-12 col-xs-12" id="blog-form" style="padding: 10px">
            <div class="panel panel-default">
                <div class="panel-heading"><span >ID : <?php echo $post->id; ?></span>
                    <input type="hidden" id="post_id" value="<?php echo $post->id; ?>">
                    <?php if($post->status == 1) {?> <span class="label label-success" style="float:right;padding: 5px;">PUBLISHED</span><?php } ?>
                    <?php if($post->status == 0) {?> <span class="label label-default" style="float:right;padding: 5px;">UNPUBLISH</span><?php } ?>
                </div>
                <div class="panel-body">
                    <div><strong>Author</strong> : <?php echo $post->author; ?></div>
                    <div><strong>Created At</strong> : <?php echo $post->created_at; ?></div>
                    <div><strong>Last Update By: </strong> : <?php echo $post->updated_by; ?></div>
                    <div><strong>Last Updated At: </strong> : <?php echo $post->updated_at; ?></div>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="" value="<?php echo $post->title; ?>" class="form-control" id="email" v-model="title">
            </div>
            <div class="form-group">
                <label for="pwd">Category:</label>
                <select name="categories" id="categories" class="form-control">
                <?php foreach ($categories as $key => $value) { ?>
                    <option <?php if($value->id == $post->category_id){ echo "selected"; } ?> value="<?php echo $value->id; ?>"><?php echo $value->category; ?></option>
                <?php } ?>
                </select>    
            </div>
            <div class="form-group">
                <label for="pwd">Publish Date:</label>
                <div class="input-group"><span id="sizing-addon2" class="input-group-addon">
                    <i class="fa fa-calendar-o"></i></span> 
                    <input type="text" name="dueDate" placeholder="" value="<?php echo $post->published_date; ?>" id="dueDate" aria-describedby="sizing-addon2" class="form-control dueDate">
                </div>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <div>
                    ...
                    <!--<input v-on:change="loadPreviewThumb" style="display: none;" type="file" id="thumbnail_file"  accept="image/x-png,image/jpeg" >-->
            ...
            <hr>
            <button class="btn btn-default btn-sm" v-on:click="updatePost()"><i class="fa fa-save"></i> Save Post</button>
