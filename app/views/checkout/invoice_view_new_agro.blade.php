        ...
        <tr>
         ...
         <tr>
            ...
            <td style="width: 30%; text-align: left; padding-bottom: 5px;">
                <font size="4"><strong>INVOICE</strong></font><br />
            </td>
        </tr>
    ...
    <table style="width: 100%;border-top: 1px solid #000;border-bottom: 1px solid #000;" class="address_container">
        <thead>
            ...
            <tr>
                ...
                <th style="width: 12%;" valign="top" class="address_container">Unit Price <br />({{ ($invoice_bussines_currency !== 'MYR' &&  $invoice_bussines_currency !== null &&  $invoice_bussines_currency !== '') ? $invoice_bussines_currency : Config::get("constants.CURRENCY") }})</th>
                <th style="width: 9%;" valign="top" class="address_container">Unit Promo Price<br />({{ ($invoice_bussines_currency !== 'MYR' &&  $invoice_bussines_currency !== null &&  $invoice_bussines_currency !== '') ? $invoice_bussines_currency : Config::get("constants.CURRENCY") }})</th>
                <th style="width: 9%;" valign="top" class="address_container">Gross Amount<br />({{ ($invoice_bussines_currency !== 'MYR' &&  $invoice_bussines_currency !== null &&  $invoice_bussines_currency !== '') ? $invoice_bussines_currency : Config::get("constants.CURRENCY") }})</th>
                <th style="width: 9%;" valign="top" class="address_container">Discount Amount<br />({{ ($invoice_bussines_currency !== 'MYR' &&  $invoice_bussines_currency !== null &&  $invoice_bussines_currency !== '') ? $invoice_bussines_currency : Config::get("constants.CURRENCY") }})</th>
        ...
        else // for normal product
        {
            // $subtotal = $subtotal + $product->total;
             ...
             if(isset($product->platform_original_price) && $product->platform_original_price > 0){
                
                $platform_unit_total  = (number_format($product->platform_original_price, 2, ".", "") * $product->unit);
                
                
                $total_excl_gst = $platform_unit_total - $product_disc;
                $total_incl_gst = $platform_unit_total - $product_disc + $gst_amount;
            ...
            if(isset($product->original_price) && $product->original_price > 0)
            {
                ...
                $sub_total_discount =  (number_format($unit_price, 2, ".", "") * $product->unit) - (number_format($promo_price, 2, ".", "") * $product->unit);
            }
            else 
            {
            ...
            if(isset($product->platform_original_price) && $product->platform_original_price > 0){
                 ...
                 $promo_price = $product->platform_original_price; 
                 $sub_total_discount =  (number_format($unit_price, 2, ".", "") * $product->unit) - (number_format($promo_price, 2, ".", "") * $product->unit);
            }
            
            $subtotal_gross_amount += $total_incl_gst;
            $total_gross_amount += (number_format($unit_price, 2, ".", "") * $product->unit );
            $total_discount_amount += $sub_total_discount;
        ...
        ?>
            <tr>
                ...
                <td style="width: 6%;" align="center" class="address_container">{{number_format($product->unit, 0, ".", "")}}</td>
                ...
                <td style="width: 10%;" align="center" class="address_container">{{ number_format(number_format($unit_price, 2, ".", "") * $product->unit, 2, ".", ",") }}</td>
                <td style="width: 10%;" align="center" class="address_container">{{ number_format(  $sub_total_discount, 2, ".", ",") }}</td>
      ...
      <!--   <tfoot> -->
            ...
            ?>
            <tr>
                <td colspan="4" style="border-bottom: 0px;" class="address_container">
                    Sub-Total
                </td>
            ...
            <tr>
                <td colspan="6" style="border-top: 0px; border-bottom: 0px;" class="address_container">
                    **Delivery Charges
                </td>
            ...
            <tr>
                <td colspan="6" style="border-top: 0px;" class="address_container">
                    **Processing Fee
                </td>
            ...
            <?php
             ...
             $jcashpnt = 0; 
            if (isset($display_jcash))
            {
                $jcashpnt = $display_jcash->jcash_point / 100; 

            ?>
            <tr>
                <td colspan="6" style="border-top: 0px;" class="address_container">
                    JCashback Points({{$display_jcash->jcash_point}}) 
                </td>
                
                <td align="center" style="border-top: 0px;" class="address_container">
                 -{{number_format($jcashpnt, 2, ".", "")}}
                </td>
            </tr>
           ...
           <!-- <tr>
            ...
            else
            {
             ...
             if($display_coupon>0){
                $subtotal_excl_gst += $delivery_charges + $process_fees - $jcashpnt;
             }
             else{
                $subtotal_excl_gst += $delivery_charges + $process_fees-$display_coupon['coupon_amount'] - $jcashpnt;
             }
           ...
           // $subtotal_excl_gst += $delivery_charges + $process_fees-$display_coupon['coupon_amount'];
            ...
            <tr>
                <td colspan="6" style="border-top: 0px;" class="address_container"><?php if ($display_points->count() || $display_coupon>0){ echo "Total";}else{ echo "Grand Total"; }?></td>
            ...
            <tr>
                <td colspan="6" style="border-top: 0px;" class="address_container">
                     <?php
                    if (isset($display_coupon['coupon_code']) && $display_coupon['coupon_code'] !='JOCOMIAN')
                    {
                    ?>
                    Coupon Code ({{$display_coupon['coupon_code']}}) 
                    <? } else { ?>
            ...
            @foreach ($display_points as $point)
                ...
                <tr>
                    <td colspan="6" style="border-top: 0px;" class="address_container">{{ $point->type }} ({{ $point->point }} points)</td>
                    <td align="center" class="address_container">
...
<!--            @if ($display_points->count())
            ...
            ?>
                ...
                <tr>
                    ...
                    <td align="center" class="address_container">
                        ...
                        $totalAmountDue = ($subtotal_excl_gst) - $total_point - $display_coupon['coupon_amount']; ?>
                        {{ number_format(abs($totalAmountDue), 2, '.', '') }} 
                    </td>
                </tr>
            <? }?>
    ...
    <table style="width: 100%;" class="address_container">
        <tr>
            ...
            <td>
                <u>Bank Details:</u><br />
                A/c Name : TIEN MING DISTRIBUTION SDN BHD	<br />
                A/c Number : 8011210779<br />
                Bank : CIMB
            </td>
        </tr>
    </table>
