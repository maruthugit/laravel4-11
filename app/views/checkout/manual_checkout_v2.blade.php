...
$total_fees_foreign = $trans_row->foreign_delivery_charges ;
$mpaysku    = '';
$items      = array();
$key        = 0;
            ...
            <div class="panel panel-default">
                <div class="list-group">
                    ...
                    <?php  
                     ...
                     ?>
                        <div class="list-group-item item">
                            <?php if($trans_row->invoice_bussines_currency != 'MYR'){ ?>
                                <span class="pull-right">
                                    {{ $trans_row->invoice_bussines_currency }} {{ number_format($row->foreign_price * $row->unit, 5, '.', '') }}
                            ...
                            SKU: {{ $row->sku }}<br>
                            <?php if($trans_row->invoice_bussines_currency != 'MYR'){ ?>
                            Unit Price: {{ $trans_row->invoice_bussines_currency }} {{ number_format($row->foreign_price, 5, '.', '') }} / MYR {{ number_format($row->price, 2, '.', '') }}<br>
                            <?php } else{ ?>
                    ...
                    <div class="list-group-item">
                        ...
                        <?php if($trans_row->invoice_bussines_currency != 'MYR'){ ?>
                            <span class="pull-right" style="text-align: left;">
                                {{ $trans_row->invoice_bussines_currency }} {{ number_format($trans_row->foreign_delivery_charges , 5, '.', '') }}
                        ...
                        ** {{ $label_delivery }}
                        ...
                        <?php if($trans_row->invoice_bussines_currency != 'MYR'){ ?>
                            <span class="pull-right" style="text-align: left;">
                                {{ $trans_row->invoice_bussines_currency }} {{ number_format($trans_row->process_fees, 2, '.', '') }}
                                <br><span style="font-size:11px;">{{ $currency }} {{ number_format($trans_row->process_fees, 2, '.', '') }}</span>
                            </span>
                        <?php } else{ ?>
                            <span class="pull-right" style="text-align: left;">{{ $currency }} {{ number_format($trans_row->process_fees, 2, '.', '') }}</span>
                        <?php } ?>
                    ...
                    <div class="list-group-item">
                        ...
                        <?php if($trans_row->invoice_bussines_currency != 'MYR'){ ?>
                            <span class="pull-right">{{ $trans_row->invoice_bussines_currency }} {{ number_format($total_fees_foreign , 5, '.', '') }}
                    ...
                    <div class="list-group-item">
                        ...
                        // OLD FORMULA
                        // $total = $total_fees - ($coupon_code != '' ? $coupon_code_amount : 0) + ($trans_row->gst_total != '' ? $trans_row->gst_total : 0) - ($total_trans_points ? $total_trans_points : 0); 
