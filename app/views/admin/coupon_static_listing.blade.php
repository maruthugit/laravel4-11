...
@section('title') Checkout Coupon @stop
@section('content')

<div id="page-wrapper">
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header"> Checkout Coupon Management
            <span class="pull-right">
            <a class="btn btn-primary" title="" data-toggle="tooltip" href="{{asset('/')}}coupon/couponstatics"><i class="fa fa-refresh"></i></a> 
            @if ( Permission::CheckAccessLevel(Session::get('role_id'), 12, 5, 'AND'))
            <a class="btn btn-primary" title="" data-toggle="tooltip" href="{{asset('/')}}coupon/addcoupon"><i class="fa fa-plus"></i></a>
            @endif
            </span></h1>
        </div>
        <!-- /.col-lg-12 -->
...
<form id="remove_frm" name="remove_frm" action="{{asset('/')}}coupon/staticremove" method="post">
...
@section('script')
    $('#dataTables-coupon').dataTable({
        ...
        "processing": true,
        "serverSide": true,
        "ajax": "{{ URL::to('coupon/listingstatic') }}",
        "order" : [[0,'desc']],
        "columnDefs" : [{
        "targets" : "_all",
        "defaultContent" : ""
        ...
        { "data" : "amount" },
        { "data" : "status" },
        { "data" : "Action", "orderable" : false, "searchable" : true, "className" : "text-center" }
        ]
    });
@stop
