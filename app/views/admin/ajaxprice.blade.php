                ...
                <div class="col-lg-12">
                    <div class="form-group">
                        <h3 class="page-header">Product name : {{$name}} (SKU: {{$sku}})
                            <span class="pull-right"><a class="btn btn-default" title="" href="/transaction/ajaxproduct/"><i class="fa fa-reply"></i> Back</a></span>
                        </h3>
                    </div>
                </div>
                <!-- /.col-lg-12 -->
                                ...
                                <table class="table table-striped table-bordered table-hover" id="dataTables-products">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="col-md-2">Promo Price (MYR)</th>
                                            <th class="col-md-1">Price (USD)</th>
                                            <th class="col-md-2">Promo Price (USD)</th>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#dataTables-products').dataTable({
                "processing" : true,
                "serverSide" : true,
                "ajax" : "{{ URL::to('transaction/pricesajax/'.$id) }}",
                "order" : [[ 0, 'desc' ]],
                "lengthMenu" : [ 5 ],
                "bLengthChange" : false,
                "columnDefs" : [{
            ...
            $(document).on("click", "#selectItem", function() {
                ...
                if( parent.$('#selected_type').val() == 2){
                    ...
                    var rowTd = $('<tr id="'+lid+'" class="product">\
                                    ...
                                    <input type="hidden" id="price_foreign" name="price_foreign[]" value="'+localStorage.data4+'">\\n\
                                    <input type="hidden" id="price_promo_foreign" name="price_promo_foreign[]" value="'+localStorage.data5+'">\\n\
                                    <td> <b> {{ $name }}</b> <br><i class="fa fa-tag"></i> {{$sku}} <br>'+ localStorage.data0 +'</td>\
                                    <td class="hidden-xs hidden-sm">'+localStorage.data1+'</td>\
                                    <td class="hidden-xs hidden-sm col-xs-1 text-right p_price">USD '+localStorage.data4+'<div style="font-size:10px;"> MYR '+localStorage.data2+'</div></td>\
                                    <td class="hidden-xs hidden-sm col-xs-1 text-right promo_price">USD '+localStorage.data5+'<div style="font-size:10px;"> MYR '+localStorage.data3+'</td>\
