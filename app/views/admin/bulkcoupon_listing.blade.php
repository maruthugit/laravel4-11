...
<div id="page-wrapper">
    <div class="row">
        <div class="col-lg-12">
            ...
            <a class="btn btn-primary" title="" data-toggle="tooltip" href="{{asset('/')}}coupon/bulk"><i class="fa fa-refresh"></i></a> 
            @if ( Permission::CheckAccessLevel(Session::get('role_id'), 12, 5, 'AND'))
            <a class="btn btn-primary" title="" data-toggle="tooltip" href="{{asset('/')}}coupon/bulkadd"><i class="fa fa-plus"></i></a>
            @endif
            </span></h1>
        </div>
        <!-- /.col-lg-12 -->
...
<form id="remove_frm" name="remove_frm" action="{{asset('/')}}coupon/bulkremove" method="post">
...
@section('script')
    $('#dataTables-coupon').dataTable({
        ...
        "processing": true,
        "serverSide": true,
        "ajax": "{{ URL::to('coupon/bulklisting') }}",
        "order" : [[0,'desc']],
        "columnDefs" : [{
        "targets" : "_all",
        "defaultContent" : ""
