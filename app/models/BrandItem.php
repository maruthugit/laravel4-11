...
class BrandItem extends Eloquent implements UserInterface, RemindableInterface {
    ...
    public static $rules = array(
        ...
        'branditem_en'        =>'required|mimes:gif,jpeg,jpg,png',
        'branditem_thumb_en'  =>'required|mimes:gif,jpeg,jpg,png',
        // 'image' => 'required|mimes:jpeg,jpg,png',
    );

    public static $messages = array(
        // 'url_link.required' => 'The URL is required.',
        'branditem_en.required'       => 'The default Brand Image is required.',
        'branditem_thumb_en.required' => 'The default Brand Thumbnail Image is required.',
    );
	 ...
	 * @var string
	 */
	protected $table = 'jocom_brand_items';
...
//            }
    ...
    public function scopeUpdateBrandItem($query, $id, array $data) {
        $banner = DB::table('jocom_brand_items')
                    ->where('id', $id)
                    ->update($data);
        return $banner;
    }
    ...
    public static function getOldFilename($id, $type, $device_type, $language) {
        ...
        $banner = DB::table('jocom_brand_items_images')
                    ->select($file_type)
                    ->where('brand_id', $id)
    ...
    public function scopeInsertBrandItemImage($query, array $inputs) {
        return DB::table('jocom_brand_items_images')
                    ->insert($inputs);
    }

    public function scopeUpdateBrandItemImage($query, $id, $language, $device_type, array $inputs) {
        return DB::table('jocom_brand_items_images')
                    ->where('brand_id', '=', $id)
                    ->where('language', '=', $language)
                    ->where('device', '=', $device_type)
    ...
    public function scopeGetBrandItemImages($query, $id) {
        return DB::table('jocom_brand_items_images')
                    ->where('brand_id', '=', $id)
                    ->get();
    }
    ...
    public static function getImagesByLanguage($id, $device_type, $language) {
        $query = DB::table('jocom_brand_items_images')
    ...
    public function scopeSet_sort($query, $id, $option) {
        $data   = array();
        $pos    = DB::table('jocom_brand_items')
                    ->orderBy('pos', 'desc')
                    ->get();
