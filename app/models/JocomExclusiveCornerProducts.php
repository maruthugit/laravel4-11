...
class JocomExclusiveCornerProducts extends Eloquent
{
    protected $table = 'jocom_jocomexcorner_products';
    
    
    public static function getTotalCampaignProduct($campaign_id){
        
        return DB::table('jocom_jocomexcorner_products')
    ...
    public function scopeStoreProducts($lid,$lbl,$actualprice,$promoprice,$productqty,$limitqty,$productid,$seq,$id) {
        ...
        $id2 = DB::table('jocom_jocomexcorner_products')
                ->insertGetId(array(
                    'fid'=>$id,
                    'label_id'=>$lid,
                    'label'=>$lbl,
       ...
       return DB::table('jocom_jocomexcorner_stock')->insert(array(
                'stock'=>$productqty, 
                'fpid'=>$id2, 
                'modify_by'=>Session::get('username'),
                'modify_date'=>date('Y-m-d H:i:s')
