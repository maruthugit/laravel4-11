...
class DynamicSaleProducts extends Eloquent
{
    protected $table = 'jocom_dynamicsale_products';
    
    
    public static function getTotalCampaignProduct($campaign_id){
        
        return DB::table('jocom_dynamicsale_products')
    ...
    public function scopeStoreProducts($lid,$lbl,$actualprice,$promoprice,$productqty,$limitqty,$productid,$seq,$id) {
        ...
        $id2 = DB::table('jocom_dynamicsale_products')
                ->insertGetId(array(
                    'fid'=>$id,
                    'label_id'=>$lid,
                    'label'=>$lbl,
       ...
       return DB::table('jocom_dynamicsale_stock')->insert(array(
                'stock'=>$productqty, 
                'fpid'=>$id2, 
                'modify_by'=>Session::get('username'),
                'modify_date'=>date('Y-m-d H:i:s')
