...
class ECommunityProducts extends Eloquent
{
    protected $table = 'jocom_ecommunity_products';
    
    
    public static function getTotalCampaignProduct($campaign_id){
        
        return DB::table('jocom_ecommunity_products')
...
//    }
    ...
    public static function getCampaignProduct($campaign_id){
        ...
        return DB::table('jocom_ecommunity_products AS JCP')
    ...
    public static function findProductOrderAfter($position_product_id,$campaign_id,$status = 1){
        ...
        return ECommunityProducts::where('order_position',">", $position_product_id)
                    ->where('status', $status)
                    ->where('campaign_id', $campaign_id)
                    ->first();
    ...
    public static function swapOrderPosition($id, $position, $campaign_id)
    {
        $product = self::find($id);
        $another_product = self::where('order_position', $position)
